...
import net.minecraft.server.v1_14_R1.PacketPlayOutBlockBreak;
import net.minecraft.server.v1_14_R1.PlayerInteractManager;

public class v1_14_R1 implements PInjector{
	...
	public void inject(Player player){
		...
		EntityPlayer player_1_14 = ((CraftPlayer)player).getHandle();
										...
										switch(packet.d().ordinal()){
											case 0: 
												new BukkitRunnable(){
													@Override
													public void run(){
														PlayerInteractManager manager = player_1_14.playerInteractManager;
														float f = default_method(manager, position, packet.c(), packet.d(), player_1_14);
														...
														if (!iblockdata.isAir() && f >= 1.0F) {
													        ...
													        if ((boolean)UPacketsInjector.reflect("e", manager,false)){
													        	...
													        	player_1_14.playerConnection.sendPacket(new PacketPlayOutBlockBreak(g, manager.world.getType(g), PacketPlayInBlockDig.EnumPlayerDigType.START_DESTROY_BLOCK, false)); 
													        }
													        ...
													        manager.world.a(player_1_14.getId(), position, (int)(f*10.0F));
													        player_1_14.playerConnection.sendPacket(new PacketPlayOutBlockBreak(position, manager.world.getType(position), packet.d(), true));
											...
											case 1:
												new BukkitRunnable(){
													...
													public void run(){
														PlayerInteractManager manager = player_1_14.playerInteractManager;
														default_method(manager, position, packet.c(), packet.d(), player_1_14);
														UPacketsInjector.input(PlayerInteractManager.class, "e", manager, false);
													    manager.world.a(player_1_14.getId(), position, -1);
													    player_1_14.playerConnection.sendPacket(new PacketPlayOutBlockBreak(position, manager.world.getType(position), packet.d(), true));
													}
												}.runTask(main);
												return;
												...
												new BukkitRunnable(){
													...
													public void run(){
														...
														if(position.equals(UPacketsInjector.reflect(PlayerInteractManager.class, "g", manager, BlockPosition.ZERO))){
															...
															if (!iblockdata.isAir()) {
																float f1 = iblockdata.getDamage(player_1_14, player_1_14.world, position) * (k + 1);
																if (f1 >= 0.7F) {
																	UPacketsInjector.input(PlayerInteractManager.class,"e", manager, false);
																	manager.world.a(player_1_14.getId(), position, -1);
																	manager.a(position, packet.d());
																	LBlockBreakFaction.instance.onBreakBlock(new BlockBreakEvent(loc.getBlock(), player));
																} 
														...
														player_1_14.playerConnection.sendPacket(new PacketPlayOutBlockBreak(position,iblockdata, packet.d(), true));
													}
												}.runTask(main);
												return;
											default: break;
	...
	private static float default_method(PlayerInteractManager manager, BlockPosition blockposition, EnumDirection enumdirection, EnumPlayerDigType packetplayinblockdig_enumplayerdigtype, EntityPlayer player){
		...
		if (manager.isCreative()) {
	        if (!manager.world.douseFire(null, blockposition, enumdirection)) {
	          manager.a(blockposition, packetplayinblockdig_enumplayerdigtype);
	        } else {
	          player.playerConnection.sendPacket(new PacketPlayOutBlockBreak(blockposition, manager.world.getType(blockposition), packetplayinblockdig_enumplayerdigtype, true));
	        } 
	        return f;
	      } 
	      if (player.a(manager.world, blockposition, ((EnumGamemode)UPacketsInjector.reflect(PlayerInteractManager.class,"gamemode", manager, EnumGamemode.NOT_SET)))) {
	        player.playerConnection.sendPacket(new PacketPlayOutBlockBreak(blockposition, manager.world.getType(blockposition), packetplayinblockdig_enumplayerdigtype, false));
	        return f;
	      }
