...
public class v1_9_R2 implements PInjector{
	...
	public void inject(Player player){
		...
		EntityPlayer player_1_9 = ((CraftPlayer)player).getHandle();
										...
										switch(packet.c().ordinal()){
											case 0:
												new BukkitRunnable(){
													@Override
													public void run(){
														PlayerInteractManager manager = player_1_9.playerInteractManager;
														if(manager.isCreative()){
															if (!player_1_9.world.douseFire(null, position, packet.b())) LBlockBreakFaction.instance.onBreakBlock(new BlockBreakEvent(loc.getBlock(), player));
															player_1_9.playerConnection.sendPacket(new PacketPlayOutBlockChange(manager.world, position)); 
														}else{
															...
															if (gamemode.c()){
														        ...
														        if(!player_1_9.cV()) {
														       		ItemStack itemstack = player_1_9.getItemInMainHand();
															...
															if(block.getBlockData().getMaterial() != Material.AIR) {
																block.attack(manager.world, position, player_1_9);
																f = iblockdata.a(player_1_9, player_1_9.world, position);
															...
															if (block.getBlockData().getMaterial() != Material.AIR && f >= 1.0F) {
														        ...
														        int i = (int)(f * 10.0F);
														        manager.world.c(player_1_9.getId(), position, i);
											...
											case 1:
												new BukkitRunnable(){
													...
													public void run(){
													    ...
													    manager.world.c(player_1_9.getId(), position, -1);
													}
												}.runTask(main);
												return;
											case 2:
