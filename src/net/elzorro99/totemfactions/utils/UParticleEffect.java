...
public enum UParticleEffect {
	...
	private static final Map<String, UParticleEffect> NAME_MAP = new HashMap<String, UParticleEffect>();
	private static final Map<Integer, UParticleEffect> ID_MAP = new HashMap<Integer, UParticleEffect>();
	...
	static {
		for (UParticleEffect effect : values()) {
	 ...
	 */
	private UParticleEffect(String name, int id, int requiredVersion, ParticleProperty... properties) {
		this.name = name;
		this.id = id;
	 ...
	 */
	public static UParticleEffect fromName(String name) {
		for (Entry<String, UParticleEffect> entry : NAME_MAP.entrySet()) {
	 ...
	 * @return The particle effect
	 */
	public static UParticleEffect fromId(int id) {
		for (Entry<Integer, UParticleEffect> entry : ID_MAP.entrySet()) {
			if (entry.getKey() != id) {
				continue;
			}
			return entry.getValue();
	 ...
	 */
	private static boolean isDataCorrect(UParticleEffect effect, ParticleData data) {
	 ...
	 */
	private static boolean isColorCorrect(UParticleEffect effect, ParticleColor color) {
	...
	public void display(float offsetX, float offsetY, float offsetZ, float speed, int amount, Location center, List<Player> players) throws ParticleVersionException, ParticleDataException, IllegalArgumentException {
		...
		if (!players.get(0).getLocation().getWorld().equals(Main.getInstance().getCurrentTotemLocation().getWorld())) {
			return;
		}
	...
	public static final class ParticlePacket {
		...
		private final UParticleEffect effect;
		 ...
		 * @see #initialize()
		 */
		public ParticlePacket(UParticleEffect effect, float offsetX, float offsetY, float offsetZ, float speed, int amount, boolean longDistance, ParticleData data) throws IllegalArgumentException {
			initialize();
			if (speed < 0) {
		 ...
		 */
		public ParticlePacket(UParticleEffect effect, Vector direction, float speed, boolean longDistance, ParticleData data) throws IllegalArgumentException {
		 ...
		 * @see #ParticleEffect(ParticleEffect, float, float, float, float, int, boolean, ParticleData)
		 */
		public ParticlePacket(UParticleEffect effect, ParticleColor color, boolean longDistance) {
			this(effect, color.getValueX(), color.getValueY(), color.getValueZ(), 1, 0, longDistance, null);
			if (effect == UParticleEffect.REDSTONE && color instanceof OrdinaryColor && ((OrdinaryColor) color).getRed() == 0) {
		...
		public static void initialize() throws VersionIncompatibleException {
			...
			try {
				...
				packetConstructor = UReflection.getConstructor(packetClass);
				getHandle = UReflection.getMethod("CraftPlayer", PackageType.CRAFTBUKKIT_ENTITY, "getHandle");
				playerConnection = UReflection.getField("EntityPlayer", PackageType.MINECRAFT_SERVER, false, "playerConnection");
				sendPacket = UReflection.getMethod(playerConnection.getType(), "sendPacket", PackageType.MINECRAFT_SERVER.getClass("Packet"));
		...
		private void initializePacket(Location center) throws PacketInstantiationException {
			...
			try {
				...
				if (version < 8) {
					...
					UReflection.setValue(packet, true, "a", name);
				} else {
					UReflection.setValue(packet, true, "a", enumParticle.getEnumConstants()[effect.getId()]);
					UReflection.setValue(packet, true, "j", longDistance);
					if (data != null) {
						int[] packetData = data.getPacketData();
						UReflection.setValue(packet, true, "k", effect == UParticleEffect.ITEM_CRACK ? packetData : new int[] { packetData[0] | (packetData[1] << 12) });
					}
				}
				UReflection.setValue(packet, true, "b", (float) center.getX());
				UReflection.setValue(packet, true, "c", (float) center.getY());
				UReflection.setValue(packet, true, "d", (float) center.getZ());
				UReflection.setValue(packet, true, "e", offsetX);
				UReflection.setValue(packet, true, "f", offsetY);
				UReflection.setValue(packet, true, "g", offsetZ);
				UReflection.setValue(packet, true, "h", speed);
				UReflection.setValue(packet, true, "i", amount);
			} catch (Exception exception) {
